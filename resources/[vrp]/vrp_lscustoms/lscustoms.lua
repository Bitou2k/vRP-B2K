...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		SetVehicleModKit(veh,0)	
		myveh.vehicle = veh
		myveh.displaymodel = GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()
		myveh.model = GetEntityModel(veh)
		...
		myveh.neon = (IsVehicleNeonLightEnabled(myveh.vehicle, 0) and IsVehicleNeonLightEnabled(myveh.vehicle, 1) and IsVehicleNeonLightEnabled(myveh.vehicle, 2) and IsVehicleNeonLightEnabled(myveh.vehicle, 3))
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		if LSCMenu.currentmenu == "main" then
			LSCMenu:showNotification("Seu veículo foi reparado.")
		else
			LSCMenu:showNotification("Item comprado.")
		end
		return true
	else
		LSCMenu:showNotification("~r~Você não pode pagar por essa compra.")
		return false
	end
end
