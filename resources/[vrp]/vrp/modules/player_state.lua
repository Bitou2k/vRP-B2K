...
local isBlackout = 0
RegisterServerEvent("b2k:setBlackout")
AddEventHandler("b2k:setBlackout", function(cond)
	isBlackout = cond
end)
...
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
  ...
  TriggerClientEvent("b2k:blackout", player, isBlackout)
    ...
    if data.customization ~= nil then
      vRPclient.setCustomization(source,{data.customization},function() -- delayed weapons/health, because model respawn
        if data.weapons ~= nil then -- load saved weapons
          ...
          if data.armour ~= nil then -- set armour
