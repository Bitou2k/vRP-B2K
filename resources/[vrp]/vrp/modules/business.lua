    ...
    for k,v in pairs(rows) do
      ...
      if row.user_id ~= nil then
        ...
        vRP.getUserIdentity(row.user_id,function(identity)
          if identity then
            menu[htmlEntities.encode(row.name)] = {function()end, lang.business.directory.info({row.capital,htmlEntities.encode(identity.nome),htmlEntities.encode(identity.sobrenome),identity.registration,identity.phone})}
          end
