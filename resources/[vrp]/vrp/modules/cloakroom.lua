...
RegisterServerEvent("b2k-character:setGender")
AddEventHandler("b2k-character:setGender", function (genderSent)
  local gender = tonumber(genderSent)
  if gender == 1 then
    vRPclient.setCustomization(source,{cfg.defaultFemale})
  else
    vRPclient.setCustomization(source,{cfg.defaultMale})
  end
end)
...
local function build_client_points(source)
  for k,v in pairs(cfg.cloakrooms) do
    ...
    if cloakroom and menu then
      ...
      -- cloakroom
      vRPclient.addMarker(source,{x,y,z,1.0,1.0,0.7,0,125,255,125,150})
