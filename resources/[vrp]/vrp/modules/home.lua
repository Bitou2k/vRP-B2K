...
local function build_entry_menu(user_id, home_name)
  ...
  menu[lang.home.sell.title()] = {function(player,choice)
    if not home_sell_cd[player] then
      home_sell_cd[player] = true
      vRP.getUserAddress(user_id, function(address)
  ...
  if user_id ~= nil then
    for k,v in pairs(cfg.homes) do
      ...
      if string.match(k, "Barraco") then
        vRPclient.addScaledBlip(source,{x,y,z,0.7,374,1,"Barraco"})
      elseif string.match(k, "Casa") or string.match(k, "Luxo") or string.match(k, "Basico")  then
        vRPclient.addScaledBlip(source,{x,y,z,0.7,374,2,"Casa"})
      elseif string.match(k, "Mansao") or string.match(k, "Mansão") then
        vRPclient.addScaledBlip(source,{x,y,z,0.7,492,5,"Mansão"})
  	  else
        vRPclient.addScaledBlip(source,{x,y,z,0.7,v.blipid,v.blipcolor,k})
  	  end
