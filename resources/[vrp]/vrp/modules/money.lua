...
function vRP.tryPayment(user_id,amount)
  if amount < 0 then
    print("**ADMIN INJECT NEGATIVE MONEY tryPayment**")
	SendWebhookMessage(ac_webhook_gameplay, "**ADMIN INJECT NEGATIVE MONEY tryPayment** \n```\nUser ID: "..user_id.." Amount: "..amount.."```")
	return false
  else
...
function vRP.tryFullPayment(user_id,amount)
  ...
  elseif amount < 0 then
    print("**ADMIN INJECT NEGATIVE MONEY tryFullPayment**")
    SendWebhookMessage(ac_webhook_gameplay, "**ADMIN INJECT NEGATIVE MONEY tryFullPayment** \n```\nUser ID: "..user_id.." Amount: "..amount.."```")
    return false
  else
	  local money = vRP.getMoney(user_id)
      ...
      if nplayer ~= nil then
        ...
        if nuser_id ~= nil then
          ...
          vRP.prompt(player,lang.money.give.prompt(),"",function(player,amount)
            ...
            if amount > 0 and vRP.tryPayment(user_id,amount) then
			  vRP.logToFile("moneySendLog.txt","Enviado Por: "..user_id.." - Para: "..nuser_id.." - Amount: " ..amount)
