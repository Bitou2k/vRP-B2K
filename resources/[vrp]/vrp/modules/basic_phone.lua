...
function vRP.sendServiceAlert(sender, service_name,x,y,z,msg)
  ...
  if service and (msg ~= nil and string.len(msg) >= 4) then
    local players = {}
	...
	if sender ~= nil then
		vRPclient.notify(sender,{"Chamados sem mensagem não serão enviados."})
	end
  end
end
    ...
    vRP.getUserByPhone(phone, function(dest_id)
      if identity and dest_id then
        ...
        if dest_src then
          ...
          if from == nil then
            from = "Numero Inválido"
          end
          if msg == nil then 
            msg = "Mensagem Inexistente"
          end
          --vRPclient.notify(dest_src,{lang.phone.sms.notify({from, msg})})
    		  vRPclient.notifyPicture(dest_src,{"CHAR_CHAT_CALL",1,from,false,lang.phone.sms.notify({from, msg})})
    		  vRPclient.playSound(dest_src,{"WEAPON_PURCHASE", "HUD_AMMO_SHOP_SOUNDSET"})
