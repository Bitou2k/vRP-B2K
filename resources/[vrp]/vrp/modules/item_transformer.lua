    ...
    if v and user_id ~= nil then -- for each player transforming
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        if new_weight > vRP.getInventoryMaxWeight(user_id) then
          ...
          vRPclient.notify(tonumber(k), {"~r~Sem reagentes suficientes."})
    		end
...
local function informer_placement_tick()
  ...
  for k,v in pairs(vRP.rusers) do
    ...
    vRPclient.setNamedMarker(player,{"vRP:informer",x,y,z,1.5,1.5,0.7,0,255,125,125,150})
...
--SetTimeout(cfg.informer.interval*60000,informer_placement_tick) -- desabilitado por enquanto
